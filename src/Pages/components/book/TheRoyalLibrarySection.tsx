...
type BooksLibrarySection = {
  title: string;
  subtitle: string;
  items: BooksLibraryItem[];
};

type TheRoyalLibrarySectionProps = {
  library?: BooksLibrarySection;
};
...
export const TheRoyalLibrarySection = ({
  ...
  const handleLibraryButtonClick = (item: BooksLibraryItem) => {
    const buttonText = item.button_text.trim().toUpperCase();
          ...
          <div className="flex flex-col items-center justify-center gap-6 sm:gap-8 lg:flex-row lg:flex-wrap xl:items-stretch xl:gap-10">
            {library.items.map((item, index) => {
              ...
              const isNotifyButton =
                item.button_text.trim().toUpperCase() === "NOTIFY ME";
              const imageSrc = getCmsAssetUrl(item.image);
